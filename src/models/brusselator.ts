  ...
  params: [
    {
      ...
      axisEligible: true, axisRange: [1, 4],
      description: "活性化因子 u の一定供給率。均一定常値 u0=A を決める。上げると u のベース濃度が増え、パターンの基準が明るくなりスポットが太くなる。下げすぎると構造が消えやすい。A_min は反応の発散を避けるため 0.5 から 1 に引き上げた。",
    },
    {
      key: "B", label: "変換率 B", symbol: "B",
      min: 1, max: 4.5, step: 0.05, default: 4.4,
      axisEligible: true, axisRange: [3, 4.5],
      description: "u を v へ変換する反応率で不安定性の主要パラメータ。1+A² のすぐ手前の帯でチューリングパターンが発生する。上げると均一状態からスポット・縞へと遷移する。上限 4.5 は A=2 の均一不安定しきい値 5 の手前に置き、explicit Euler の発散を避ける (旧上限 12 は小 A 隅で如何なる実用 dt でも発散したため引き下げた)。",
    },
    {
    ...
    },
    {
      ...
      description: "活性化因子 u の拡散係数。正規化 9 点ラプラシアン (固有値域 [-2,0]) では O(1) の大きさが必要で、Gray-Scott の Du(最大1.2)と同程度に取る。上げるとパターンスケールが大きく滑らかになり、下げると細かい構造になる。Dv=Du·ratio に直結する (旧既定 0.1 では既定点で模様が出なかったため引き上げた)。",
    },
  ],
  defaultXKey: "B",
  defaultYKey: "A",
